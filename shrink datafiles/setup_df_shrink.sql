...
CREATE OR REPLACE PROCEDURE ANALYZE_DATAFILE_SHRINKABILITY(ANALYZE_OBJECT_TYPE IN VARCHAR2 DEFAULT 'TABLESPACE', ANALYZE_OBJECT_NAME IN VARCHAR2 DEFAULT 'ALL_TABLESPACES') AS
...
BEGIN
	...
	IF v_analyze_obj_type = 'TABLESPACE' THEN
		BEGIN
			...
			v_file_name := v_file_name || '_' || v_analyze_obj_name || '.sql';
	...
	ELSIF v_analyze_obj_type = 'DATAFILE' THEN
		BEGIN
			SELECT DIRECTORY_PATH INTO v_dir_path FROM dba_directories WHERE DIRECTORY_NAME = v_dir_name;
			file_handle := UTL_FILE.FOPEN(v_dir_name,v_file_name||'.sql','W',32000);
			UTL_FILE.PUT_LINE(file_handle, '--Script Generated at ' || v_start_time);
			UTL_FILE.FCLOSE(file_handle);
		EXCEPTION WHEN OTHERS THEN
			v_todir := FALSE;
		...
		END;
		ANALYZE_DATAFILE(v_analyze_obj_name, v_dir_name, v_file_name);
	END IF;
